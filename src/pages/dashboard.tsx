...
import { useConferenceCount, usePublicationCount, useCitationCount } from 'lib/roq';

import { ResearchListPage as ShowCaseList } from 'pages/research';

function DashboardPage() {
  const { session } = useSession();

  const conferenceStat = useConferenceCount();

  const publicationStat = usePublicationCount();

  const citationStat = useCitationCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Conferences',
      result: conferenceStat?.data,
      isLoading: conferenceStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Publications',
      result: publicationStat?.data,
      isLoading: publicationStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Citations',
      result: citationStat?.data,
      isLoading: citationStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
