...
import { useRoqClient, useCollaborationFindFirst } from 'lib/roq';
...
function CollaborationEditPage() {
  ...
  const { data, error, isLoading, mutate } = useCollaborationFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: CollaborationInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.collaboration.update({
        data: values as RoqTypes.collaboration,
        where: {
          id,
        },
      });
