...
import { useRoqClient, useCollaborationFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<CollaborationInterface, unknown>;

interface CollaborationListPageProps {
...
export function CollaborationListPage(props: CollaborationListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useCollaborationFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.collaboration.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: CollaborationInterface) => {
    if (hasAccess('collaboration', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/collaborations/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/collaborations/view/${record.id}`} passHref legacyBehavior>
                <Button
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="collaboration" />
        </Flex>
