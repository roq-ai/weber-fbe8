...
import { useRoqClient, useCitationFindFirst } from 'lib/roq';
...
function CitationEditPage() {
  ...
  const { data, error, isLoading, mutate } = useCitationFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: CitationInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.citation.update({
        data: values as RoqTypes.citation,
        where: {
          id,
        },
      });
