...
import { useRoqClient, useWorkshopFindFirst } from 'lib/roq';
...
function WorkshopEditPage() {
  ...
  const { data, error, isLoading, mutate } = useWorkshopFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: WorkshopInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.workshop.update({
        data: values as RoqTypes.workshop,
        where: {
          id,
        },
      });
